<script lang="ts" setup>
const { loading, message, setErrorMessage, alertType } =
  useStateRequestHelper();
const snackbar = useSnackbar();

const { DeclineOrderSchema } = useSchema();
const { t: $t, locale } = useI18n();

import { useTimeoutFn } from "@vueuse/core";
const props = defineProps<{
  orderId: string;
}>();

const emit = defineEmits(["close", "reload"]);

const { start, stop } = useTimeoutFn(() => {
  reloadData();
}, 1000);

function closeModal() {
  emit("close");
}

const reason = ref();

async function declineOrder() {
  loading.value = true;
  const { error } = await useCustomFetch(
    `/admins/orders/${props.orderId}/decline?lang=${locale.value}`,
    {
      method: "POST",
      body: {
        reason: reason.value,
      },
    }
  );
  if (error.value) {
    setErrorMessage(error.value?.data?.message);
  } else {
    await snackbar.clear();
    await snackbar.add({
      type: "success",
      text: $t("decline_order_success_txt"),
      groupKey: new Date().toISOString(),
    });
    start();
  }

  loading.value = false;
}

function reloadData() {
  emit("reload");
}

onMounted(() => {
  stop();
});
</script>

<template>
  <div>
    <VeeForm @submit="declineOrder" :validation-schema="DeclineOrderSchema">
      <div class="card card-compact">
        <div class="card-body">
          <Alert v-model="message" :type="alertType" />
          <p>{{ $t("decline_order_confirm_msg") }}</p>
          <h3 class="text-2xl font-medium">
            {{ props.orderId }}
          </h3>
          <div>
            <FormGroup name="reason" label="Reason">
              <FormSpTextarea
                name="reason"
                v-model="reason"
                placeholder="eg: The order is not provide valid data"
                class="textarea-bordered w-full"
              />
            </FormGroup>
          </div>
        </div>

        <div class="card-actions flex justify-between items-center">
          <div>
            <button
              class="btn btn-sm h-11 normal-case"
              @click="closeModal"
              :disabled="loading"
              type="button"
            >
              {{ $t("cancel_txt") }}
            </button>
          </div>
          <div>
            <button
              type="submit"
              :disabled="loading"
              class="btn btn-sm btn-error h-11 normal-case"
            >
              {{ $t("decline_order_txt") }}
            </button>
          </div>
        </div>
      </div>
    </VeeForm>
  </div>
</template>

<style></style>
